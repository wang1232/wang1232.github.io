...
* 索引是在数据库表的字段上添加的，是为了提高查询效率存在的一种机制，一张表的一个字段可以添加一个索引，索引相当于一本书的目录，是为了缩小扫描范围而存在的一种机制。
	* 索引在表中并不是一个具体的ID，而**是一个数据结构**，用于加速数据库表中数据的检索速度。索引可以看作是一个目录，它帮助数据库系统快速定位到表中的特定数据行，而不需要扫描整个表。
	* 索引可以包含一个或多个列的值，这些列的值被存储在索引结构中（如B树、哈希表等），以便快速查找。当执行查询时，数据库系统可以使用索引来快速定位到包含所需数据的行，而不是扫描整个表。
	* 在大多数数据库系统中，索引会包含指向表中行的指针或行ID，以便在找到索引条目后能够快速检索到对应的数据行。
	* 此外，有些数据库系统（如MySQL的InnoDB存储引擎）中的**主键索引（也称为聚集索引）确实会以一种特殊的方式与表中的行数据结合存储。**在这种情况下，**表中的数据实际上是按照主键索引的顺序物理存储的，**主键索引的叶子节点直接包含了行的数据。但是，这并不意味着索引本身就是ID，而是说主键索引在物理存储上与表的数据有特殊的关联。
