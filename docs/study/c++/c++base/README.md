...
初始化是指在声明变量时给变量赋予一个初始值。初始化可以在声明变量的同时进行，也可以在声明后通过赋值语句进行。
...
* 对于类和复杂类型来说，这两者最主要的区别便是有没有创建新的对象和是否调用构造函数。
...
隐式转换指的是在没有显式类型转换语句的情况下，编译器自动将一个类型的值转换成另一个类型的值。这种转换通常发生在混合类型的操作中，比如将整数赋值给浮点数变量，或者将较小的整数类型赋值给较大的整数类型。隐式转换可能会导致精度丢失或数据溢出。

```c++
	// 隐式转换：将整数隐式转换为浮点数
    float f = 10; // 隐式将整数10转换为浮点数10.0
...
* 智能指针就是类，类中有一个成员，管理着原始指针，负责自动释放动态分配的对象，防止堆栈溢出。（用于管理动态分配对象的生命周期）
...
* 多个shared_ptr实例**共享它指向的对象，多个shared_ptr指向相同（关联）的对象**，在**内部采用计数机制**来实现。
...
std::shared_ptr<A> p0 = std::make_shared<A>("西施");  //常用
...
* 直接创建`std::shared_ptr`：
	* 可以通过`std::make_shared`函数或`std::shared_ptr`的构造函数直接创建并初始化`std::shared_ptr`。更高效，可以一次性分配控制块和对象内存，并减少内存泄漏的风险。
* 从原始指针创建`std::shared_ptr`
	* 有时需要从已有的原始指针（如`new`分配的指针）创建`std::shared_ptr`。此时，应确保原始指针不再被其他代码直接管理，以避免双重删除。从原始指针创建`std::shared_ptr`时，要确保原始指针的唯一性，避免双重管理导致的未定义行为。
...
// 直接创建，标准初始化：调用shared_ptr的构造函数
shared_ptr<int> ptr1 = std::make_shared<int>(10);   		// 标准初始化
std::shared_ptr<int> ptr3 = ptr1;  							// 隐式拷贝，计数+1
ptr1 = ptr3; 												// 赋值：ptr1接管ptr3的对象，ptr3的引用计数减少
std::cout << "ptr3: " << *ptr3 << std::endl;

// 从原始指针创建std::shared_ptr
int* rawPtr = new int(30);
shared_ptr<A> pa0(new A("name_zhangsan"));				   	// 匿名初始化
shared_ptr<A> pa2(p0);  									// 用已经存在的shared_ptr初始化pa2，计数+1
shared_ptr<A> p2 = p1; 										// 隐式拷贝，计数+1
std::shared_ptr<int> ptr3(rawPtr);		// 注意：此时rawPtr不应再被使用，因为ptr3已经接管了它的所有权

// 复制与赋值
shared_ptr<A> pa1 = pa0; //计数+1	复制构造，ptr1和ptr4共享同一个对象
shared_ptr<A> pa2 = pa0;
...
* `std::shared_ptr`支持自定义删除器，允许在对象销毁时执行自定义的删除逻辑。
* 自定义删除器为`std::shared_ptr`提供了更灵活的内存管理机制，适用于需要特殊删除逻辑的场景。
...
std::shared_ptr<int> ptr6(new int(40), deleter);
// 当ptr6的生命周期结束时，会调用自定义的deleter来删除对象
```
...
* **引用计数操作是线程安全的**
	...
	* std::shared_ptr 的引用计数（即管理对象的共享所有权的计数）是线程安全的。因为std::shared_ptr 的内部引用计数是**原子的**，这意味着多个线程可以安全地对同一个 std::shared_ptr 对象进行引用计数的操作（如 shared_ptr 的拷贝构造和赋值）。这些操作不会导致数据竞争。
	...
	// 回调函数
	void increment_count(vector<shared_ptr<int>> &sp_ar){
	    for(int i = 0;i<N;i++){
			sp_ar[i] = p;
	    }
	}
	...
	**最终引用计数：**
	...
	这里的关键在于每次赋值操作都会**原子地增加引用计数**。因此，即使两个线程同时执行 sp_arr[i] = p;，也不会导致数据竞争或未定义行为。

* **线程不安全的部分**

	* **对象访问**
		* 尽管 std::shared_ptr 的引用计数是线程安全的，但对**所管理对象的访问并不是线程安全的**。如果多个线程同时访问同一个 shared_ptr 管理的对象，并且至少有一个线程在修改该对象，那么就需要**额外的同步机制（如互斥锁）**来确保线程安全。
		* 下面的代码运行，输出的结果不是预想的20000，每次运行输出的结果都会发生变化。因此同时修改shared_ptr指向的对象不是线程安全的。
...
lambda本质就是一个匿名函数/闭包，用于定义并创建匿名的函数对象，可以方便的让别的函数调用。
...
![image-20241013110952160](全栈_C++.assets/image-20241013110952160.png)
